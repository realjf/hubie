...
namespace Hubie
{
	...
	void HierarchyWindow::DrawNode(entt::entity node, entt::registry& registry)
	{
		...
		if(show)
		{
			...
			if(payload != NULL && payload->IsDataType("Drag_Entity"))
			{
				...
				HB_ASSERT(payload->DataSize == sizeof(entt::entity*), "Error ImGUI drag entity");
	...
	void HierarchyWindow::OnImGui()
	{
		...
		m_SelectUp = Input::Get().GetKeyPressed(Hubie::InputCode::Key::Up);
		m_SelectDown = Input::Get().GetKeyPressed(Hubie::InputCode::Key::Down);
