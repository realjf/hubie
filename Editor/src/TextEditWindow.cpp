...
namespace Hubie
{
	...
	void TextEditWindow::OnImGui()
	{
		...
		ImGui::Text("%6d/%-6d %6d lines  | %s | %s | %s | %s", cpos.mLine + 1, cpos.mColumn + 1, editor.GetTotalLines(), editor.IsOverwrite() ? "Ovr" : "Ins", editor.CanUndo() ? "*" : " ", editor.GetLanguageDefinition().mName.c_str(), Hubie::StringUtilities::GetFileName(m_FilePath).c_str());
