...
{
	Scene::OnInit();
	Application::Get().GetSystem<HubiePhysicsEngine>()->SetDampingFactor(0.998f);
	Application::Get().GetSystem<HubiePhysicsEngine>()->SetIntegrationType(IntegrationType::RUNGE_KUTTA_4);
	Application::Get().GetSystem<HubiePhysicsEngine>()->SetBroadphase(Hubie::CreateRef<OctreeBroadphase>(5, 3, Hubie::CreateRef<SortAndSweepBroadphase>()));
...
void GraphicsScene::LoadModels()
{
	...
	Hubie::Ref<Graphics::Mesh> terrain = Hubie::Ref<Graphics::Mesh>(new Terrain());

	auto material = Hubie::CreateRef<Graphics::Material>();
