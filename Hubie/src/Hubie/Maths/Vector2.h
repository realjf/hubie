...
namespace Hubie::Maths
{
	...
	public:
		...
		float AbsDotProduct(const Vector2& rhs) const { return Hubie::Maths::Abs(x * rhs.x) + Hubie::Maths::Abs(y * rhs.y); }
		...
		Vector2 Abs() const { return Vector2(Hubie::Maths::Abs(x), Hubie::Maths::Abs(y)); }

		/// Linear interpolation with another vector.
		Vector2 Lerp(const Vector2& rhs, float t) const { return *this * (1.0f - t) + rhs * t; }

		/// Test for equality with another vector with epsilon.
		bool Equals(const Vector2& rhs, float eps = M_EPSILON) const { return Hubie::Maths::Equals(x, rhs.x, eps) && Hubie::Maths::Equals(y, rhs.y, eps); }

		/// Return whether is NaN.
		bool IsNaN() const { return Hubie::Maths::IsNaN(x) || Hubie::Maths::IsNaN(y); }
...
{
	template <>
	struct hash<Hubie::Maths::Vector2>
	{
		size_t operator()(const Hubie::Maths::Vector2& x) const
		{
