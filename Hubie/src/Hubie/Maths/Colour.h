...
namespace Hubie::Maths
{
	...
	public:
		...
		Colour Abs() const { return Colour(Hubie::Maths::Abs(r_), Hubie::Maths::Abs(g_), Hubie::Maths::Abs(b_), Hubie::Maths::Abs(a_)); }

		/// Test for equality with another Colour with epsilon.
		bool Equals(const Colour& rhs, float eps = M_EPSILON) const
		{
			return Hubie::Maths::Equals(r_, rhs.r_, eps)
				&& Hubie::Maths::Equals(g_, rhs.g_, eps)
				&& Hubie::Maths::Equals(b_, rhs.b_, eps)
				&& Hubie::Maths::Equals(a_, rhs.a_, eps);
		}
