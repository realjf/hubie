...
namespace Hubie::Graphics
{
    ...
    Maths::Vector2 ToHubieVector(const ofbx::Vec2& vec)
    {
        return Maths::Vector2(float(vec.x), float(vec.y));
    }

    Maths::Vector3 ToHubieVector(const ofbx::Vec3& vec)
    {
        return Maths::Vector3(float(vec.x), float(vec.y), float(vec.z));
    }

    Maths::Vector4 ToHubieVector(const ofbx::Vec4& vec)
    {
        return Maths::Vector4(float(vec.x), float(vec.y), float(vec.z), float(vec.w));
    }

    Maths::Vector4 ToHubieVector(const ofbx::Color& vec)
    {
        return Maths::Vector4(float(vec.r), float(vec.g), float(vec.b), 1.0f);
    }

    Maths::Quaternion ToHubieQuat(const ofbx::Quat& quat)
    {
    ...
    void Model::LoadFBX(const std::string& path)
    {
        ...
        for(int i = 0; i < c; ++i)
        {
            ...
            if(material)
            {
                ...
                properties.albedoColour = ToHubieVector(material->getDiffuseColor());
                properties.metallicColour = ToHubieVector(material->getSpecularColor());
