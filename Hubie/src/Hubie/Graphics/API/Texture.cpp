...
namespace Hubie
{
	namespace Graphics
	{
		...
		TextureFormat Texture::BitsToTextureFormat(uint32_t bits)
		{
			...
			default:
				HB_ASSERT(false, "[Texture] Unsupported image bit-depth! ({0})", bits);
				return TextureFormat::RGB8;
			}
		}
		...
		{
			HB_ASSERT(CreateFunc, "No Texture2D Create Function");
		...
		{
			HB_ASSERT(CreateFromSourceFunc, "No Texture2D Create Function");
		...
		{
			HB_ASSERT(CreateFromFileFunc, "No Texture2D Create Function");
		...
		{
			HB_ASSERT(CreateFunc, "No TextureCube Create Function");
		...
		{
			HB_ASSERT(CreateFromFileFunc, "No TextureCube Create Function");
		...
		{
			HB_ASSERT(CreateFromFilesFunc, "No TextureCube Create Function");
		...
		{
			HB_ASSERT(CreateFromVCrossFunc, "No TextureCube Create Function");
		...
		{
			HB_ASSERT(CreateFunc, "No TextureDepth Create Function");
		...
		{
			HB_ASSERT(CreateFunc, "No TextureDepthArray Create Function");
