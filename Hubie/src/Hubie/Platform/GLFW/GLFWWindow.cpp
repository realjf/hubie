...
namespace Hubie
{
	...
	{
		HB_ERROR("GLFW Error - {0} : {1}", error, description);
	}
	...
	bool GLFWWindow::Init(const WindowProperties& properties)
	{
		...
		if (!s_GLFWInitialized)
		{
			...
			HB_ASSERT(success, "Could not initialize GLFW!");
		...
		glfwSetWindowIconifyCallback(m_Handle, [](GLFWwindow* window, int32_t state)
			{
				...
				case GL_FALSE:
					...
					break;
				default:
					HB_INFO("Unsupported window iconify state from callback");
				}
			});
		...
		glfwSetKeyCallback(m_Handle, [](GLFWwindow* window, int key, int scancode, int action, int mods)
			{
				...
				case GLFW_PRESS:
				{
					KeyPressedEvent event(GLFWKeyCodes::GLFWToHubieKeyboardKey(key), 0);
					data.EventCallback(event);
					break;
				}
				case GLFW_RELEASE:
				{
					KeyReleasedEvent event(GLFWKeyCodes::GLFWToHubieKeyboardKey(key));
					data.EventCallback(event);
					break;
				}
				case GLFW_REPEAT:
				{
					KeyPressedEvent event(GLFWKeyCodes::GLFWToHubieKeyboardKey(key), 1);
					data.EventCallback(event);
					break;
				}
				}
		...
		glfwSetMouseButtonCallback(m_Handle, [](GLFWwindow* window, int button, int action, int mods)
			{
				...
				case GLFW_PRESS:
				{
					MouseButtonPressedEvent event(GLFWKeyCodes::GLFWToHubieMouseKey(button));
					data.EventCallback(event);
					break;
				}
				case GLFW_RELEASE:
				{
					MouseButtonReleasedEvent event(GLFWKeyCodes::GLFWToHubieMouseKey(button));
					data.EventCallback(event);
					break;
				}
				}
		...
		glfwSetCharCallback(m_Handle, [](GLFWwindow* window, unsigned int keycode)
			{
				...
				KeyTypedEvent event(GLFWKeyCodes::GLFWToHubieKeyboardKey(keycode), char(keycode));
				data.EventCallback(event);
			});
		...
		HB_INFO("Initialised GLFW version : {0}", glfwGetVersionString());
