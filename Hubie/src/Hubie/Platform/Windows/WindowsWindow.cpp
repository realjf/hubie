...
#define HID_USAGE_GENERIC_KEYBOARD ((USHORT)0x06)
    ...
    static HICON createIcon(unsigned char* image, int width, int height, int xhot, int yhot, bool icon)
    {
        ...
        if(!color)
        {
            HB_ERROR("Win32: Failed to create RGBA bitmap");
            return NULL;
        }
        ...
        if(!mask)
        {
            HB_ERROR("Win32: Failed to create mask bitmap");
            DeleteObject(color);
            return NULL;
        }
        ...
        if(!handle)
        {
            if(icon)
            {
                HB_ERROR("Win32: Failed to create icon");
            }
            else
            {
                HB_ERROR("Win32: Failed to create cursor");
            }
        }
    ...
    void WindowsWindow::SetIcon(const std::string& filePath, const std::string& smallIconFilePath)
    {
        ...
        if(filePath != "")
        {
            ...
            uint8_t* pixels = Hubie::LoadImageFromFile(filePath, &width, &height, nullptr, nullptr, true);
        ...
        {
            uint32_t width, height;
            uint8_t* pixels = Hubie::LoadImageFromFile(smallIconFilePath, &width, &height, nullptr, nullptr, true);
