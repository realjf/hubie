...
namespace Hubie
{
    ...
    SceneManager::~SceneManager()
    {
        ...
        if(m_CurrentScene)
        {
            HB_INFO("[SceneManager] - Exiting scene : {0}", m_CurrentScene->GetSceneName());
    ...
    void SceneManager::SwitchScene(const std::string& name)
    {
        ...
        {
            HB_ERROR("[SceneManager] - Unknown Scene Alias : {0}", name.c_str());
        }
    }
    ...
    void SceneManager::ApplySceneSwitch()
    {
        ...
        {
            HB_ERROR("[SceneManager] - Invalid Scene Index : {0}", m_QueuedSceneIndex);
            m_QueuedSceneIndex = 0;
        }
        ...
        {
            HB_INFO("[SceneManager] - Exiting scene : {0}", m_CurrentScene->GetSceneName());
            app.GetSystem<HubiePhysicsEngine>()->SetPaused(true);
        ...
        app.GetSystem<HubiePhysicsEngine>()->SetDefaults();
        app.GetSystem<B2PhysicsEngine>()->SetDefaults();
        app.GetSystem<HubiePhysicsEngine>()->SetPaused(false);

        std::string physicalPath;
        if(Hubie::VFS::Get()->ResolvePhysicalPath("//Scenes/" + m_CurrentScene->GetSceneName() + ".lsn", physicalPath))
        {
        ...
        HB_INFO("[SceneManager] - Scene switched to : {0}", m_CurrentScene->GetSceneName().c_str());
    ...
    void SceneManager::EnqueueScene(Scene* scene)
    {
        ...
        HB_INFO("[SceneManager] - Enqueued scene : {0}", scene->GetSceneName().c_str());
    }
