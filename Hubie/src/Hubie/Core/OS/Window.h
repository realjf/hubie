...
namespace Hubie
{
	struct HB_EXPORT WindowProperties
	{
		WindowProperties(uint32_t width = 1280, uint32_t height = 720, int renderAPI = 0, const std::string& title = "Hubie", bool fullscreen = false, bool vSync = true, bool borderless = false, const std::string& filepath = "")
			: Width(width)
			, Height(height)
			, Title(title)
