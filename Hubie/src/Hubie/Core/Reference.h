...
#include "Hubie/Core/OS/Memory.h"

namespace Hubie
{
	class HB_EXPORT RefCount
	{
	public:
		RefCount();
		~RefCount();
	...
	public:
		...
		inline Reference(const Reference<U>& moving) noexcept
		{
			...
			{
				HB_ERROR("Failed to cast Reference");
			}
		}
	...
	private:
		...
		{
			HB_ASSERT(ptr, "Creating shared ptr with nullptr");
	...
	public:
		...
		explicit WeakReference(T* ptr) noexcept
			: m_Ptr(ptr)
		{
			HB_ASSERT(ptr, "Creating weak ptr with nullptr");
...
{
	template <typename T>
	struct hash<Hubie::Reference<T>>
	{
		size_t operator()(const Hubie::Reference<T>& x) const
		{
			return hash<T*>()(x.get());
		}
	};
