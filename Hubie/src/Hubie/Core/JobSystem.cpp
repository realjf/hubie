    ...
    namespace System
    {
        ...
        namespace JobSystem
        {
            ...
            void OnInit()
            {
                ...
                numThreads = Hubie::Maths::Max(1U, numCores - 1);
                ...
                for(uint32_t threadID = 0; threadID < numThreads; ++threadID)
                {
                    std::thread worker([threadID]
                        {
                            ...
                            HB_PROFILE_SETTHREADNAME(ss.str().c_str());
                    ...
                    HB_ASSERT(affinity_result > 0, "");

                    // Increase thread priority:
                    BOOL priority_result = SetThreadPriority(handle, THREAD_PRIORITY_HIGHEST);
                    HB_ASSERT(priority_result != 0, "");
                    ...
                    HB_PROFILE_SETTHREADNAME(wss.str().c_str());
#endif

                    worker.detach();
                }

                HB_INFO("Initialised JobSystem with [{0} cores] [{1} threads]", numCores, numThreads);
            }
